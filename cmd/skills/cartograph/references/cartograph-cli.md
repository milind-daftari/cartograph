...
Start the background service. Detaches by default and prints PID/address
when ready. The service holds graphs in memory, handles embedding jobs,
and exposes a Streamable HTTP MCP endpoint at `/mcp`.
...
cartograph serve start --no-idle

# Start in foreground (for debugging)
cartograph serve start --no-detach

# Start without MCP endpoint
cartograph serve start --no-mcp
```
...
- The MCP endpoint at `/mcp` is enabled by default and uses the Streamable HTTP transport (stateless mode).

## MCP Server

### `cartograph mcp`

Start an MCP (Model Context Protocol) server over stdin/stdout for AI
editor integration. This command blocks and communicates via JSON-RPC.
The editor (Cursor, Claude Code, OpenCode, etc.) launches this as a
subprocess.

**Opportunistic delegation:** If a background service is running, the
`mcp` command delegates all tool calls to it via HTTP (reusing warm
caches and avoiding duplicate in-memory graphs). If no service is
available, it falls back to an in-process MemoryClient.
...
```jsonc
// Cursor / Claude Code / OpenCode — stdio mode
{
  "mcpServers": {
    "cartograph": {
      "command": "cartograph",
...
```jsonc
// Alternative: point directly at the serve endpoint (requires service running)
// Use `cartograph serve start` first, then configure the editor to use
// the /mcp endpoint URL (e.g. http://localhost:<port>/mcp for TCP,
// or the unix socket path).
```

**Preferred setup:** Start the background service (`cartograph serve start`)
and use `cartograph mcp` as the editor subprocess. The `mcp` command will
automatically delegate to the running service, giving you warm caches,
embedding support, and shared state.
...
### `cartograph skills install [path]`

Install cartograph skills for AI coding agents.

**Flags:**
- `--agent, -a <id,...>` — Agent ID(s) to install for (non-interactive). Use `all` for all agents.
- `--upgrade, -u` — Update only already-installed targets. Used by package manager post-install hooks (e.g. Homebrew).
- `--no-global` — Install to project-local directory instead of global.
...
# Update only existing installations (e.g. after brew upgrade)
cartograph skills install --upgrade

# Install to a specific directory
cartograph skills install /path/to/skills
```

### `cartograph skills uninstall [path]`

Remove cartograph skills from AI coding agents.

**Flags:**
- `--agent, -a <id,...>` — Agent ID(s) to uninstall from. Use `all` for all agents.
- `--no-global` — Uninstall from project-local directory instead of global.
